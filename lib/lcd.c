...
void lcd_init() {
    ...
    // Set all palette colors to the default color
    for (int i = 0; i < 4; i++) {
...
u8 lcd_read(u16 address) {
    ...
    // Cast context into a byte array
    u8 *p = (u8 *)&ctx;

    // Return byte array at offset
    return p[offset];
}
...
void lcd_write(u16 address, u8 value) {
    ...
    // Cast context into a byte array
    u8 *p = (u8 *)&ctx;
    // Write value to byte array at offset
    p[offset] = value;
